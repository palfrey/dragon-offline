angular.module('dragon-offline.gamecontroller', [])

.controller('GameController', function($scope, $stateParams, $log, $ionicNavBarDelegate) {
	$scope.gameId = $stateParams.gameId;
	$scope.rows = [[]]

	$log.info($stateParams)
	$db.get("game-" + $scope.gameId)
	.then(function(game) {
		$log.info("game" + game);
		$db.get("uid-" + game["white_player"])
		.then(function(white_name) {
			$log.info("white_name = " + white_name);
			$db.get("uid-" + game["black_player"])
			.then(function(black_name) {
		...
		sgf.gameTrees[0].nodes.forEach(function(element, index, array) {
			for (k in element) {
				...
				if (k == "SZ") {
					...
					board = _.map(_.range(size), function(x) { return new Array(size)});
					$log.info(board);
				}
				if (k == "B" || k == "W") {
					var v = element[k];
